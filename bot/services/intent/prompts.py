...
def build_system_prompt(
    ...
    dt_str = current_datetime.strftime("%d.%m.%Y %H:%M")
...
2. create_event — создать событие в календаре
   {{"type": "create_event", "title": "...", "starts_at": "ISO datetime", "ends_at": "ISO datetime или null", "reminder_minutes": число или null}}

3. create_reminder — создать напоминание (без события в календаре)
...
9. get_briefing — показать утренний брифинг
   {{"type": "get_briefing"}}

10. get_analytics — показать аналитику
    {{"type": "get_analytics", "period": "week|month"}}

11. ai_chat — свободный диалог (нет конкретного действия)
...
- Все даты в ISO 8601 формате с учётом текущей даты/времени выше
- "сегодня", "завтра", "в пятницу" — конвертируй в абсолютные даты
